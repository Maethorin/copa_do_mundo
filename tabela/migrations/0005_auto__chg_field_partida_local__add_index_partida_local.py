...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Renaming column for 'Partida.local' to match new field type.
        db.rename_column('partidas', 'local', 'local_id')
        # Changing field 'Partida.local'
        db.alter_column('partidas', 'local_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['tabela.Estadio'], null=True))
        # Adding index on 'Partida', fields ['local']
        db.create_index('partidas', ['local_id'])


    def backwards(self, orm):
        # Removing index on 'Partida', fields ['local']
        db.delete_index('partidas', ['local_id'])


        # Renaming column for 'Partida.local' to match new field type.
        db.rename_column('partidas', 'local_id', 'local')
        # Changing field 'Partida.local'
        db.alter_column('partidas', 'local', self.gf('django.db.models.fields.CharField')(max_length=200, null=True))
